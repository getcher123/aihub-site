...
**Status:** Active
**Description:** Desktop (≥xl/1280px) uses the original absolute-positioned layout verbatim. Below xl, a completely separate flex/grid layout renders using a `mobile` prop passed to card components. This avoids breakage of pixel-perfect desktop designs while giving full control over mobile.
**Implementation:** `hidden xl:block` for desktop wrapper, `xl:hidden` for mobile wrapper in Home.tsx. Each section component accepts `{ mobile?: boolean }` and renders the appropriate variant.
**Rationale:** The original design uses absolute px positioning that can't be made responsive with media queries alone. Dual layout keeps desktop untouched.
