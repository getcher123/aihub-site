...
### [2026-03-15] — Replace absolute layout with responsive Grid/Flex

**Status:** Accepted
**Context:** Original Anima export used `absolute` positioning with fixed px values (min-w-[1920px], left-[389px], etc.), completely unusable below 1920px viewport.
**Decision:** Rewrite Home component to use CSS Grid for two-column layout + Flexbox for card internals, with Tailwind responsive prefixes for all breakpoints.
**Alternatives Considered:**
- CSS `scale()` on the whole container — breaks text readability and interaction targets.
- Only adding media queries to reposition absolute elements — too fragile, doesn't truly reflow.
**Consequences:** Fully responsive 375px–2560px+. Decorative SVG overlays hidden on mobile for clarity.
