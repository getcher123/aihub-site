...
### Responsive Integration Card Pattern

**Category:** Architecture
**Status:** Active
**Description:** Integration category cards (CRM, Cloud Apps, LLM, etc.) use a shared `IntegrationCard` component with `group` hover wrapper. Layout uses CSS Grid (`grid-cols-1 md:grid-cols-2`) for the two-column split and Flexbox for inner column items.
**Implementation:** Card gradient applied via `.card-gradient` utility class in `tailwind.css`; hover brightening via `.group:hover .card-gradient`. Responsive sizes use `sm:`, `md:`, `lg:` Tailwind prefixes.
**Rationale:** Replaced absolute-positioned pixel layout for full responsiveness.
