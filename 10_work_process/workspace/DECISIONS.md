...
### [2026-03-15] — Dual layout strategy for timeline responsiveness

**Status:** Accepted
**Context:** The timeline had fixed absolute desktop coordinates and broke on small screens.
**Decision:** Keep the exact desktop layer under `lg:block` and add a dedicated `lg:hidden` mobile layout.
**Alternatives Considered:**
- Single fluid layout for all breakpoints — rejected due to desktop pixel drift.
- Pure scale transform for mobile — rejected due to readability and overflow issues.
**Consequences:** Desktop visuals stay stable while mobile gets a purpose-built readable timeline.
