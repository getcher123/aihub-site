...
### 2026-03-15 — CSS Grid with named areas for card layout

**Status:** Accepted
**Context:** Layout used absolute positioning with fixed pixel offsets (`min-w-[1920px]`), completely unusable below 1920 px.
**Decision:** Replace with CSS Grid using `grid-template-areas` at the ≥1140 px breakpoint to replicate the original 4-column placement, 2-column at ≥640 px, and single-column on mobile.
**Alternatives Considered:**
- Flexbox with order utilities — harder to replicate the exact 2D layout.
- Tailwind `grid-cols-*` without named areas — positions less explicit.
**Consequences:** Desktop appearance matches original; layout now works from 320 px up. Card widths are fluid instead of fixed.
