...
<todo id="mobile-hamburger">
Add hamburger menu for mobile nav
The navigation links are hidden on small screens — need a burger toggle to reveal them.
</todo>

<todo id="mobile-nav-items">
Add anchor links or smooth scroll for nav items
Nav items are static divs/links with no href targets — wire them to actual sections or add scroll behavior.
</todo>

<todo id="touch-cta-animation">
Test CTA border animation on touch devices
`:hover` pseudo-class doesn't work on tap — consider using `:active` or JS touch events for the conic-gradient spin.
</todo>
