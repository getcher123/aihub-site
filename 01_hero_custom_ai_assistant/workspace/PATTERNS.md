...
### Dual-Layout Responsive Pattern
**Category:** Architecture
**Status:** Active
**Description:** Desktop (≥1280px / `xl:`) uses a pixel-perfect absolute-positioned layout matching the Figma spec. Below that breakpoint, a completely separate flex-based layout renders using responsive Tailwind utilities (`sm:`, `md:`, `lg:`). Both live in the same component, toggled by `hidden xl:block` / `xl:hidden`.
**Rationale:** The desktop design has precise absolute offsets that can't gracefully scale. Maintaining two layout branches avoids compromise on either end.

### CTA Button Animated Border
**Category:** Architecture
**Status:** Active
**Description:** The CTA button uses a `::before` pseudo-element with `conic-gradient(from var(--angle))` and `@property --angle` to animate the gradient angle from 0→360deg on hover. The `mask-composite: exclude` trick creates a border-only effect. Desktop uses `.cta-btn-wrapper`, responsive uses `.cta-btn-responsive` (inset:0 variant).
**Rationale:** CSS `@property` enables smooth angle interpolation; `forwards` fill keeps the gradient at 360deg after animation ends.
