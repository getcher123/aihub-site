...
**Status:** Active
**Description:** Page-level components use flex column→row with Tailwind breakpoints (`lg:flex-row`). Content stacks on mobile, reflows side-by-side on desktop. Max-width container (`max-w-7xl`) centers content. All sizing in rem/% — no fixed px widths on layout containers.
**Rationale:** Avoids absolute positioning brittleness. Single-source responsive via Tailwind utility classes.
